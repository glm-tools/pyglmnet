        ...
        for env in self.envs:
            ...
            if env == "pyglmnet":
                ...
                if distr in ["gaussian", "poisson"]:
                    res[env]["score"] = r2_score(y_test, y_test_hat)
                elif distr == "binomial":
                    res[env]["score"] = accuracy_score(
                        y_test, (y_test_hat > 0.5).astype(int)
                    )
            ...
            if env == "statsmodels":
                ...
                y_test_hat = model.predict(
                    statsmodels_res.params, exog=sm.add_constant(X_test)
                )
