...
def fetch_tikhonov_data(dpath='/tmp/glm-tools'):
    ...
    url = 'https://raw.githubusercontent.com/glm-tools/datasets/master/tikhonov/fixations.csv'
    cmd = 'curl -o ' + fname + ' ' + url
    subprocess.call(cmd, shell=True)
    fixations_df = pd.read_csv(fname)

    fname = dpath+'/' + 'probes.csv'
    url = 'https://raw.githubusercontent.com/glm-tools/datasets/master/tikhonov/probes.csv'
    cmd = 'curl -o ' + fname + ' ' + url
    subprocess.call(cmd, shell=True)
    probes_df = pd.read_csv(fname)

    fname = dpath+'/' + 'spiketimes.csv'
    url = 'https://raw.githubusercontent.com/glm-tools/datasets/master/tikhonov/spiketimes.csv'
    cmd = 'curl -o ' + fname + ' ' + url
    subprocess.call(cmd, shell=True)
